...
/**
 * Created by Koma on 18/05/15.
 */
public class TripsLoader extends RichFlatMapFunction<String, Tuple4<String, Integer, Integer, Integer>> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        public void flatMap(String s, Collector<Tuple4<String, Integer, Integer, Integer>> out) throws Exception {
            ...
            String pickupDate;
            int pickupTime, n_row, n_column;
            Double longitudepoint, latitudepoint;

            String[] lineSplit = s.split(",");
            //Skip the first line of the csv file
            if(!lineSplit[0].equals("medallion")) {

                longitudepoint = Double.parseDouble(lineSplit[10]);
                latitudepoint = Double.parseDouble(lineSplit[11]);
                ...
                if ((latitudepoint >= southLimit) && (latitudepoint <= nordLimit) && (longitudepoint <= eastLimit) && (longitudepoint >= westLimit)) {
                    ...
                    if(((Integer.parseInt(hourSplit[1]) > 30)||((Integer.parseInt(hourSplit[1]) == 30)&&(Integer.parseInt(hourSplit[2]) > 30)))&&(Integer.parseInt(hourSplit[0])!=23)){
                        pickupTime=Integer.parseInt(hourSplit[0])+1;
                    }else
                        pickupTime=Integer.parseInt(hourSplit[0]);
                    pickupTime= (int) Math.round((((100 * (pickupTime) / 23) * (double) hourchunknumber) / 100));

                    out.collect(new Tuple4<String, Integer, Integer, Integer>(pickupDate, pickupTime, n_row, n_column));
